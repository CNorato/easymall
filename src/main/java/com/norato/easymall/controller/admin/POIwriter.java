...
public class POIwriter {
     ...
     * @return 写入数据后的工作簿对象
     */
    public static Workbook exportData(List<Product> dataList) {
        // 生成xlsx的Excel
        ...
        //构建每行的数据内容
        int rowNum = 1;
        for (Iterator<Product> it = dataList.iterator(); it.hasNext(); ) {
...
//    }
    ...
    private static void convertDataToRow(Product data, Row row) {
        ...
        // 描述
        cell = row.createCell(cellNum++);
        cell.setCellValue(null == data.getCategory() ? "" : data.getCategory());
        // 分类
        cell = row.createCell(cellNum++);
        cell.setCellValue(null == data.getDescription() ? "" : data.getDescription());
        // 价格
        cell = row.createCell(cellNum++);
        cell.setCellValue(data.getPrice());
        // 销量
        cell = row.createCell(cellNum++);
        cell.setCellValue(data.getSoldnum());
    }
}
